...
const OnColumnsModal = () => {
  ...
  const handleClick = (column, table) => {
    if (table === currentParent) {
            selectedColumnOne ?
            setSelectedColumnOne(null) : setSelectedColumnOne(column);
    } else if (table === addedParent) {
            selectedColumnTwo ?
            setSelectedColumnTwo(null) : setSelectedColumnTwo(column);
    }
  };
    ...
    <div className="on-columns-modal">
      ...
      <div className="tableOne">
        ...
        { tableOneMap.map((column, index) => (
          <button
            className={
              `${selectedColumnOne === column.column ?
              'flowButton clicked' : 'flowButton'}`}
            key={index}
      ...
      <div className="tableTwo">
        ...
        {tableTwoMap.map((column, index) => (
          <button
            className={`${selectedColumnTwo === column.column ?
                'flowButton clicked' : 'flowButton'}`}
            key={index}
